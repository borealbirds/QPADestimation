...
author: "Elly Knight (ecknight@ualberta.ca)"
...
described in Solymos et al. [1]. We refer to this approach as the *QPAD approach*.
...
Models 9--14 include the new variable `DSLS` and were introduced in QPAD v3. Models 15--29 are identical to models 1--14 but with the inclusion of the survey method `TM` variable.
...
After the package is installed, one can load the estimates. Version 2, 3, or 4
...
QPAD v3 included results for `r length(v3$.BAMCOEFS$spp)`,
v4 includes results for `r length(v4$.BAMCOEFS$spp)`.
As part of QPAD v4, we are providing parameter estimates for the
following bird species (* indicates the
`r length(v4$.BAMCOEFS$spp) - length(v3$.BAMCOEFS$spp)`
species that were not part of QPAD v3
species list):
...
plot(est34$sra4.pc, est34$sra4.spt,
     ...
     main="Effect of ARU-1SPT method on availability",
     ylim=c(0,3))
abline(0,1, lty=2, col="lightblue")
...
plot(est34$sra4.pc, est34$sra4.spm,
     ...
     ylab="ARU-1SPM availability estimate",
          main="Effect of ARU-1SPM method on availability",
     ylim=c(0,3))
abline(0,1, lty=2, col="lightblue")
...
The middle row depicts constant singing rate as a function of sampling duration (left) and constant detection distance based distance-decay function (right). Separate lines for each method are predicted for singing rate if the best supported time-removal model includes human point count vs ARU tag method.
The bottom row shows the date/time relationship for probability
of availability based on best supported time-removal model (left), and tree cover vs. land cover relationship based on best supported distance sampling model (right).

We can plot the updated v4 results for the species, to compare sample sizes, model support, general relationships between different versions, and the effect of inclusion of method on availability (note: some species' results might not be available in v3).
Sample sizes for time-removal and distance sampling can be different because the conditional maximum likelihood estimation uses different observations depending on methodologies (i.e. availability of multiple duration or distance intervals in a given survey).
...
A general and thorough comparison of the results from the different versions is
