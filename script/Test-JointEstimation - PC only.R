...
#no NONE tag method
#no na TM
bird.use <- dplyr::filter(bird, TM=="PC")

visit.use <- dplyr::filter(visit, TM=="PC")
...
for(i in 1:nrow(spp)){
  ...
  if(nrow(bird.i) > 0){
    ...
    names(mod.list) <- c("tau-null_phi-null")
  ...
  save(joint, file=file.path(root, "Results/jointestimates_multibin.Rdata")) 
...
sp <- rownames(models)[rowSums(models) > 0]
length(sp)
...
save(.BAMCOEFSjoint, file=file.path(root, "Results/BAMCOEFS_QPAD_joint_multibin.rda"))
saveRDS(bamcoefs, file=file.path(root, "Results/BAMCOEFS_QPAD_joint_multibin.rds"))

.BAMCOEFSjoint <- readRDS(file.path(root, "Results/BAMCOEFS_QPAD_joint.rds"))
...
for(i in 1:length(sp)){
  ...
  edrj <- exp(.BAMCOEFSjoint$estimates[[i]]$"tau-null_phi-null"$coefficients[1])/10
  nj <- .BAMCOEFSjoint$n[i]
  estj <- rbind(estj,
                data.frame(sraj=sraj, edrj=edrj, nj=nj, species=sp[i]))
}

estnull <- est34 %>% 
  dplyr::select(species, sra4, sra4.pc, edr4) %>% 
  left_join(estj) %>% 
  mutate(weird = ifelse(log(edrj) > 1, 1, 0)) %>% 
  dplyr::filter(!is.na(weird))

`#Null estimates----
ggplot(estnull) +
  geom_point(aes(x=sra4.pc, y=sraj, label=species)) +
#  geom_smooth(aes(x=sra4.pc, y=sraj), method="lm") +
...
ggsave(file.path(root, "Figures", "JointModelComparison - phi - null - weird.jpeg"), width = 12, height = 4, units="in")
...
ggsave(file.path(root, "Figures", "JointModelComparison - EDR - weird.jpeg"), width = 12, height = 4, units="in")

ggplot(estnull) +
  geom_point(aes(x=nj, y=log(edrj))) +
  xlab("Sample size") +
  ylab("Tau - joint model") +
  theme_bw()

ggsave(file.path(root, "Figures", "JointModelComparison - EDR - logscale.jpeg"), width = 6, height = 4, units="in")

#5. TM estimates----
estj <-data.frame(edrj.tauTM.PC=rep(NA, length(sp)), edrj.tauTM.SPT=rep(NA, length(sp)), edrj.tauTM.SPM=rep(NA, length(sp)), sraj.tauTM.all=rep(NA, length(sp)), edrj.phiTM.all=rep(NA, length(sp)), sraj.phiTM.PC=rep(NA, length(sp)), sraj.phiTM.SPT=rep(NA, length(sp)), sraj.phiTM.SPM=rep(NA, length(sp)), edrj.bothTM.PC=rep(NA, length(sp)), edrj.bothTM.SPT=rep(NA, length(sp)), edrj.bothTM.SPM=rep(NA, length(sp)), sraj.bothTM.PC=rep(NA, length(sp)), sraj.bothTM.SPT=rep(NA, length(sp)), sraj.bothTM.SPM=rep(NA, length(sp)))
for(i in 1:length(sp)){
...
ggplot(estTM) +
  geom_point(aes(x=sra4.pc, sraj.phiTM.PC, colour=factor(weird))) +
  geom_abline(intercept = 0, slope = 1)

ggplot(estTM) +
  geom_point(aes(x=sra4.spt, sraj.phiTM.SPT, colour=factor(weird)) +
  geom_abline(intercept = 0, slope = 1) +
  ylim(c(0,5))

ggplot(estTM) +
  geom_point(aes(x=sra4.spm, sraj.phiTM.SPM, colour=factor(weird))) +
  geom_abline(intercept = 0, slope = 1) +
  ylim(c(0,5))

#PHI - vs QPAD4 - with method on both sides----
ggplot(estTM) +
  geom_point(aes(x=sra4.pc, sraj.bothTM.PC, colour=factor(weird))) +
  geom_abline(intercept = 0, slope = 1)

ggplot(estTM) +
  geom_point(aes(x=sra4.spt, sraj.bothTM.SPT, colour=factor(weird))) +
  geom_abline(intercept = 0, slope = 1) +
  ylim(c(0,5))

ggplot(estTM) +
  geom_point(aes(x=sra4.spm, sraj.bothTM.SPM, colour=factor(weird))) +
  geom_abline(intercept = 0, slope = 1) +
  xlim(c(0,5)) +
  ylim(c(0,5))
